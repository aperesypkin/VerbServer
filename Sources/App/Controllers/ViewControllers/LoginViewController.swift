...
final class LoginViewController: RouteCollection {
    ...
    func boot(router: Router) throws {
        let loginRoute = router.grouped("login")
        let authSessionsMiddleware = User.authSessionsMiddleware()
        let authRoute = loginRoute.grouped(authSessionsMiddleware)
        authRoute.get(use: loginHandler)
        authRoute.post(User.self, use: loginPostHandler)
    }
